...
setwd("/Users/jacoblillelund/Documents/methods-3/ASSIGNMENT_1") # Set WD
...
```{r rename_variables}
demo_train <- clean_names(demo_train) # clean names using janitor
LU_train <- clean_names(LU_train) # clean names using janitor
token_train <- clean_names(token_train) # clean names using janitor

demo_train <- rename(demo_train, subj = child_id) # rename cols as appropriate
...
```{r extract_digits}
LU_train$visit <- str_extract(LU_train$visit, '\\d') # Extract the first digit
LU_train$visit <- as.numeric(LU_train$visit)

token_train$visit <- str_extract(token_train$visit, '\\d') # Extract the first digit
...
```{r remove_points}
# Use str_replace_all to remove all dots from the column
...
demo_train <- demo_train %>% select(1:4, 7, 9, 14, 22, 24, 33) 
LU_train <- LU_train %>% select(subj,visit,mot_mlu,chi_mlu)
token_train <- token_train %>% select(1:4,6:7)
...
```{r merge_data}
# Left join demo_train with LU_train on subj and visit
merged_data <- demo_train %>%
  left_join(LU_train, by = c("subj", "visit"))

# Left join the merged_data with token_train on subj and visit
merged_data <- merged_data %>%
  left_join(token_train, by = c("subj", "visit"))

# Check dataframe 
diagnose(merged_data) # check missing values on the final table
diagnose(demo_train) # compare with missing values on original demo_train - equal number of NA/s.
  # there are 20 less rows in LU_train and token_train datasets

# Vosialize missing values per variable
plot_na_pareto(merged_data, main = "Missing values")
```
...
# Remove all values in relevant columns where visit != 1
merged_data <- merged_data %>%
...
merged_data <- merged_data %>% 
  mutate(across("gender", ~ifelse(gender == 1, "M", "F")))

# Rename diagnosis labels
merged_data <- merged_data %>% 
  mutate(across("diagnosis", ~ifelse(diagnosis == "A", "ASD", "TD")))

# Convert variables to categorical
merged_data$gender <- factor(merged_data$gender)
merged_data$diagnosis <- factor(merged_data$diagnosis)

# Final inspection of data set
...
```{r save_changes}
write_csv(merged_data, "language_development_data.csv")
```
