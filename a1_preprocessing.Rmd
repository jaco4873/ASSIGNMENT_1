...
df_LU$VISIT <- as.numeric(df_LU$VISIT)


df_token <- data_token 
df_token$VISIT <- str_extract(df_token$VISIT, "\\d")
df_token$VISIT <- as.numeric(df_token$VISIT)
...
df_demo_LU <- left_join(df_demo_sub, df_LU_sub, by = c("SUBJ","VISIT"))

df <- left_join(df_demo_LU, df_token_sub, by = c("SUBJ","VISIT"))
...
#dplyr::recode() even more intuitive--same syntax 
