...
### Write your code here
demo_data <- read_csv("demo_train.csv")
ut_length <- read_csv("LU_train.csv")
...
```{r}
#diagnose_report(demo_data) - didn't understand how it works
```
...
##For DF2
ut_length <- clean_names(ut_length) #clean names to see the difference is it makes #it doesn't??

ut_length <- as_tibble(ut_length) #converting to a tibble from a list #unnecessary?

ut_length$visit <- str_extract(ut_length$visit, '\\d') #extracting the number
...
ut_length$visit <- as.numeric(ut_length$visit)
...
```{r remove_points}
#Removing all dots for DF1
demo_data$child_id <- str_replace_all(demo_data$child_id, "\\.", "")
```


```{r functions that don't work}
#Trial 1 to make a function - doesn't work

dot_remove = function (demo_data){
  demo_data$child_id <- str_replace_all(demo_data$child_id, "\\.", "")
}

dot_remove(ut_length)
# 
# # Trial 2 - also doesn't work :c
 df=NULL
 dot_function <- function (df) {
   df$child_id <- str_replace_all(df$child_id, "\\.", "")

 }

dot_function(ut_length)
```


```{r remove_points}
#Removing the dots for DF2 and DF3

ut_length$child_id <- str_replace_all(ut_length$child_id, "\\.", "")
word_data$child_id <- str_replace_all(word_data$child_id, "\\.", "")
...
### Write your code here
sub_demo_df <- subset(demo_data, select =c ('child_id', 
                                              'visit', 
                                              'Diagnosis', 
                                              'Ethnicity', 
                                              'Gender', 
...
#DF 2
sub_ut_df <- subset(ut_length, select =c ('child_id', 
                                              'visit','mot_mlu', 'chi_mlu'))
head(sub_ut_df)

#DF3

sub_word_df <- subset(word_data, select =c ('child_id', 
                                              'visit', 'types_MOT', 
                                              'types_CHI', 
                                              'tokens_MOT', 
                                              'tokens_CHI'))
...
#with lef_join - 372 rows
sub_merge <- left_join(sub_demo_df, sub_ut_df, by = c("child_id", "visit"))
sub_merge
#with merge - 352 rows
sub_merge2 <- merge(sub_demo_df, sub_ut_df, by= c("child_id", "visit"))
sub_merge2


###Join sub_merge and DF3 

final_merge <- left_join(sub_merge, sub_word_df, by = c("child_id", "visit")) #372 rows
...
### Here I just remove all the rows where visit is not 1

final_df <- filter(final_merge, final_merge$visit== '1')
...
#Replacing Gender column with the one where 1 is f and 2 is M using mutate and str_replace_all

final_df <- final_df %>% 
    mutate(Gender = str_replace_all(Gender, "1", "F")) %>% 
    mutate(Gender = str_replace_all(Gender, "2", "M")) 
...
```{r save_changes}
### Write your code here
write.csv(final_df, "/Users/ulya/Documents/METHODS_3/ASSIGNMENT_1/final_data.csv")
...
#Going through steps 6-9 without removing all the rows where visit is not 1

#6 # Remove all values in relevant columns where visit != 1 - stolen from Jacob
final_large <- final_merge %>%
  mutate(across(7:10, ~ifelse(visit != 1, NA, .)))
```
...
```{r}
#8
write.csv(final_large_df, "/Users/ulya/Documents/METHODS_3/ASSIGNMENT_1/final_large_df.csv")
```
