...
data_demo <- read_csv("demo_train.csv")
data_LU <- read_csv("LU_train.csv")
data_token <- read_csv("token_train.csv")
```
...
class(data_demo$VISIT)
class(data_LU$VISIT)
class(data_token$VISIT)

df_LU <- data_LU %>%
    mutate(VISIT=ifelse(grepl("visit", VISIT, ignore.case = TRUE),
                        as.numeric(sub("visit", "", VISIT, ignore.case = TRUE)),
                        as.numeric(VISIT)))
...
df_LU_points <- df_LU %>%
    mutate(SUBJ = str_replace_all(SUBJ,"(?<![A-Z])\\.", "" ))

df_token_points <- df_token %>%
    mutate(SUBJ = str_replace_all(SUBJ,"(?<![A-Z])\\.", "" ))

head(df_LU_points,50)
head(df_token_points,50)
head(data_demo,50)
...
df_demo_sub <- df_demo_sub %>%
    rename(NonVerbalIQ = MullenRaw, VerbalIQ = ExpressiveLangRaw , SocialSkills = Socialization)
...
```{r}
head(df_token_sub,50)
```


```{r merge_data}

df_demo_LU <- merge(df_demo_sub, df_LU_sub, by = c("SUBJ","VISIT"))

df <- merge(df_demo_LU, df_token_sub, by = c("SUBJ","VISIT"))
...
first_visit <- df %>%
    select(SUBJ, VISIT,ADOS, NonVerbalIQ, VerbalIQ, SocialSkills) %>%
    filter(VISIT == 1) %>%
    rename(ADOS1 = ADOS, NonVerbalIQ1 = NonVerbalIQ, VerbalIQ1 = VerbalIQ, SocialSkills1 = SocialSkills)


first_visit <- first_visit %>%
    select(SUBJ,ADOS1, NonVerbalIQ1, VerbalIQ1, SocialSkills1)


merged_df <- merge(df,first_visit, by = "SUBJ", all = TRUE)
...
class(merged_df$Gender) 
class(merged_df$Diagnosis)

merged_df <- merged_df %>%
  mutate(Gender = ifelse(Gender == 1, "F", ifelse(Gender == 2, "M", Gender)))

merged_df <- merged_df %>%
  mutate(Diagnosis = ifelse(Diagnosis == "A", "ASD", ifelse(Diagnosis == "B", "TD", Diagnosis)))
...
write.csv(merged_df, file = "final_dataset.csv")
